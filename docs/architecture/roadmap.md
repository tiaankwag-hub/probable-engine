...
| 4 | Snapshots + What Changed + trends + Issues/Incidents | Snapshot capture job, current-vs-previous comparison, "What Changed?" view, trend charts. Also absorbs Issues and Incidents from the brief, which Milestone 0's roadmap never assigned a milestone to — they pair naturally with change narratives (an incident is exactly the kind of event a "what changed" view should surface). |
